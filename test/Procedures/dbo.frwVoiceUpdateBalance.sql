...
CREATE  PROCEDURE [dbo].[frwVoiceUpdateBalance]
...
IF((@Address IS NOT NULL) AND (@Sender IS NOT NULL) AND (@username IS NOT NULL) AND (@password IS NOT NULL) AND (@message IS NOT NULL)
     AND (@sendto IS NOT NULL)) Or (@SID IS NOT NULL and @Address IS NOT NULL)
 BEGIN
    ...
    if  @ErrCode = '0' 
			begin
			 Select StringValue  from frwParseJSON(@ResponseText) where Name like 'Balance'
				UPDATE frwConfig 
				   SET  VoiceBalance = (Select StringValue  from frwParseJSON(@ResponseText) where Name like 'Balance') 
					WHERE GUID=@strGUID
			end
		else
			UPDATE frwConfig Set  VoiceBalance = @ErrCode WHERE GUID=@strGUID
