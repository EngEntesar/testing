...
CREATE FUNCTION [dbo].[frwToJSON]
(
      @Hierarchy frwJSONHierarchy READONLY
)
 
/*
the function that takes a frwJSONHierarchy table and converts it to a JSON string
...
DECLARE @MyfrwJSONHierarchy frwJSONHierarchy -- to pass the frwJSONHierarchy table around
insert into @MyfrwJSONHierarchy select * from dbo.ParseXML(@XMLSample)
SELECT dbo.frwToJSON(@MyfrwJSONHierarchy)
  ...
  WHILE @ii>0
    ...
    SELECT 
        ...
        WHEN 'object' then '  {'+@CrLf+SPACE(@indent+2)
           +'@Object'+CONVERT(VARCHAR(5),TheRow.[OBJECT_ID])+@CrLf+SPACE(@indent+2)+'}'
        WHEN 'string' THEN '"'+dbo.frwJSONEscaped(TheRow.StringValue)+'"'
